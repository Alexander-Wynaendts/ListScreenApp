...
# Install dependencies (assuming non-root installation)
...
# Unpack Chrome instead of using dpkg (no superuser privileges)
ar x /opt/render/chrome/google-chrome-stable_current_amd64.deb
tar -xf control.tar.xz -C /opt/render/chrome/
tar -xf data.tar.xz -C /opt/render/chrome/

# Find the latest ChromeDriver version that matches Chrome version
